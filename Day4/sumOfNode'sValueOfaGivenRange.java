...
public class Main {
    ...
    // Current node is below range → answer only lies in right subtree
    if (root.val < start)
        return sumOfValuesInRange(root.right, start, end);

    // Current node is above range → answer only lies in left subtree
    if (root.val > end)
        return sumOfValuesInRange(root.left, start, end);

    // Current node is within range → include it, explore both sides
    return root.val
